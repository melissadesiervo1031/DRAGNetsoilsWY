...
soilsdat5<-subset(soilsdat, Depth==5)

##summarize ph EC and Moisture by date and by disturbance##

soilsdat5_avg<-soilsdat15%>% group_by(DATE, Disturbance) %>% dplyr::summarise(meanpH=mean(pH, na.rm=TRUE),sdpH=sd(pH, na.rm=TRUE),sepH=sd(pH, na.rm=TRUE)/sqrt(n()), n=n(),meanEC=mean(EC, na.rm=TRUE),sdEC=sd(EC, na.rm=TRUE),seEC=sd(EC, na.rm=TRUE)/sqrt(n()), meanMoist=mean(Moisture, na.rm=TRUE),sdMoist=sd(Moisture, na.rm=TRUE),seMoist=sd(Moisture, na.rm=TRUE)/sqrt(n()), n=n())


## plot average pH over time series###

phTS<-ggplot(data=soilsdat5_avg)+geom_point(aes(factor(DATE), meanpH, group=Disturbance, color=Disturbance))+ylab("Soil pH")+xlab("")+ theme_classic()+ geom_errorbar(aes(x=DATE, ymin=meanpH-sepH, ymax=meanpH+sepH), color="dark gray", size=0.25) 


## plot average EC over time series###

ECTS<-ggplot(data=soilsdat5_avg)+geom_point(aes(DATE, meanEC, group=Disturbance, color=Disturbance))+ylab("Soil EC")+xlab("")+ theme_classic()+ geom_errorbar(aes(x=DATE, ymin=meanEC-seEC, ymax=meanEC+seEC), color="dark gray", size=0.25) 


### plot average moisture over time series ###

MoistTS<-ggplot(data=soilsdat5_avg)+geom_point(aes(DATE, meanMoist, group=Disturbance, color=Disturbance))+ylab("Soil Moisture")+xlab("")+ theme_classic()+ geom_errorbar(aes(x=DATE, ymin=meanMoist-seMoist, ymax=meanMoist+ seMoist), color="dark gray", size=0.25) 
